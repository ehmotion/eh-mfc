...
void handle_signal(int signum) {
	...
	case SIGINT:
		...
		if (signal_received) {
			printf("#%.3f:Signal received again, force exiting\n", get_fms());
			exit(1);
		}
		if (signum == SIGTERM)
			printf("#%.3f:Received SIGTERM, stopping...\n", get_fms());
		else
			printf("#%.3f:Received SIGINT, stopping...\n", get_fms());
...
int set_prio()
{
   ...
   */
	...
	if (0) //set affinity on CPU0
	{
		...
		printf("#%.3f:High prio set on CPU 0\n", get_fms());
	}
	return 0;
}
...
int main(int argc, char **argv)
{
	...
	const struct option long_options[] = {
		...
		{"mfcxtractor_ip", required_argument, &lopt, 7},
		{0, 0, 0, 0}
	};
	while ((opt = getopt_long(argc, argv, optstring, long_options, &loidx)) != -1) {
		...
		case 6:
			...
			break;
		case 7:
			extern int client_init (const char *ipaddr);
			client_init (optarg);
			break;
