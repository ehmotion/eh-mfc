...
void *hotplug_monitor(void *arg __attribute__((unused))) {
	printf("Start hotplug_monitor thread, thread id(%ld)\n", gettid());
	while(true) {
		usleep(100 * 1000);
...
void send_data(uint8_t endpoint, uint8_t attributes, uint8_t *dataptr,
	...
	case USB_ENDPOINT_XFER_INT:
		result = libusb_interrupt_transfer(dev_handle, endpoint, dataptr, length, &transferred, USB_SENDINT_TMO);

		if (transferred != length)
			fprintf(stderr, "Incomplete Interrupt transfer on EP%02x\n", endpoint);
		if (result == LIBUSB_SUCCESS && (0||verbose_level > 2))
			printf("Sent %d bytes (Int) to libusb EP%02x\n", transferred, endpoint);
		break;
	}
	if (verbose_level > 2 && result != LIBUSB_SUCCESS) {
