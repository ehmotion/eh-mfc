    ...
    if (poll(fds, nfds, -1) > 0) {
      for (i = 0; i < nfds; ++i) {
        ...
        if (fds[i].revents & POLLIN) {
          ...
          {
            //printf("\n#i:RD fd %d user %d", fds[i].fd, sources[fds[i].fd].user);
            return;
          }
          else
          {
            //printf("\n#w:!RD fd %d user %d", fds[i].fd, sources[fds[i].fd].user);
          }
        }
        if (fds[i].revents & POLLOUT) {
          if (sources[fds[i].fd].fp_write(sources[fds[i].fd].user))
          {
            //printf("\n#i:WR fd %d user %d", fds[i].fd, sources[fds[i].fd].user);
            return;
          }
          else
          {
            //printf("\n#w:!WR fd %d user %d", fds[i].fd, sources[fds[i].fd].user);
          }
        }
      }
    }
